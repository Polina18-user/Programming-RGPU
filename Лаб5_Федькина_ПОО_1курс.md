...
|---|---|-----|
|*fp|FILE|Указатель на переменную типа FILE|
|*src|char|Указатель на переменную типа FILE на файл source|
|*dest|char|Указатель на переменную типа FILE на файл destination|
|buffer|char|Строка данных|
|bytes|size_t|Количество элементов в строке|
### Код программы
```c
 #include <stdio.h>
 #include <stdlib.h>
